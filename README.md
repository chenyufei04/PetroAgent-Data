...
| 数据预处理 | 页面与上传功能已预留 | 支持 Excel、Word、CSV、PDF、图片等上传到独立目录；预留标准化、缺失、失真、异常和陈旧数据算法流程 |
...
│  └─ types.ts
├─ uploads/                       # 质控上传文件保存目录
├─ preprocessing-uploads/         # 数据预处理文件独立保存目录
├─ logs/
...
侧栏新增“数据预处理”标签页，接受 Excel、CSV、Word、PDF、文本、JSON、XML 和常见图片。文件保存到独立的 `preprocessing-uploads/`，刷新后从后端恢复列表，删除时同步删除后台文件。

页面预留结构与字段识别、标准化与单位换算、缺失数据处理、失真与异常检测、陈旧数据检测、结果复核与导出六个步骤。当前仅完成上传管理，“开始预处理”按钮保持禁用，等待真实数据后再接入算法。

数据库模板位于 `backend/config/database.example.toml`。实际部署时复制为 `backend/config/database.toml`，并通过环境变量提供连接信息。真实配置文件已加入 `.gitignore`，禁止提交数据库密码。

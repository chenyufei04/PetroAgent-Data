...
截至2026-08-28，当前版本已经通过完整的 Vue、TypeScript 和 Vite 生产构建验证，FastAPI 后端模块也已通过导入检查。
...
6. 对实际可获取图片进行格式、清晰度、来源、模板、文字类型和标注情况的数据摸底。
7. 建立脱敏OCR测试集，分别评估扫描表格、工程报告、仪表照片和图件的识别准确率。
8. 接入图像预处理、OCR文字/表格识别及低置信度人工复核，并将结构化结果交给现有规则引擎。
9. 将地层CT、岩心照片中的孔隙、裂缝等视觉分析作为独立计算机视觉任务，不与OCR混为一体。
10. 在确定性规则稳定后增加大模型语义质控。

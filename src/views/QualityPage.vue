...
interface QualityFile { id:string; kind:"file"|"text"; file:File; text:string; extractedText?:string; status:"waiting"|"parsing"|"ready"|"error"; saveStatus:"saving"|"saved"|"deleting"|"save-error"; savedPath?:string; savedFilename?:string; error?:string; deleteError?:string; }
...
async function saveUploadedFile(item:QualityFile):Promise<string|undefined>{
  ...
  try{const response=await fetch("/api/quality/upload",{method:"POST",body:formData});const result=await response.json() as {filename?:string;path?:string;extracted_text?:string;detail?:string};if(!response.ok)throw new Error(result.detail??`保存失败（${response.status}）`);item.savedPath=result.path;item.savedFilename=result.filename;item.saveStatus="saved";return result.extracted_text;}
...
async function uploadFiles(){
  ...
  const added:QualityFile[]=stagedFiles.value.map(file=>({id:crypto.randomUUID(),kind:"file",file,text:"",status:"waiting",saveStatus:"saving"}));
  ...
  <section class="quality-page">
    ...
    <main class="quality-right">
      ...
      <template v-else>
        ...
        <div v-else-if="activeFile.status==='waiting'" class="quality-loading">内容已进入待校验列表，请点击左侧对应项目的“校验”按钮。</div>

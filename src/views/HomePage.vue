...
import { baseRecords, termRecords } from "../data/seed";
import { useDataStore } from "../stores/data";
const store = useDataStore();
const approvedCount = computed(() => { let saved:Record<string,string>={}; try{saved=JSON.parse(localStorage.getItem("petroleum-term-review-statuses")??"{}");}catch{saved={};} return termRecords.filter(item=>(saved[item.id]??item.status)==="已审核").length; });
const distribution = computed(() => [
  {label:"术语",value:store.termCount,color:"#168bd2"},
  {label:"规则",value:store.ruleCount,color:"#7956e8"},
  {label:"基础数据",value:baseRecords.length,color:"#e49a38"},
  {label:"已审核术语",value:approvedCount.value,color:"#20ad7a"},
]);
...
<div class="overview-grid"><article><span>T</span><strong>{{store.termCount}}</strong><small>术语总数</small></article><article><span>R</span><strong>{{store.ruleCount}}</strong><small>规则总数</small></article><article><span>D</span><strong>{{baseRecords.length}}</strong><small>基础数据</small></article><article><span>✓</span><strong>{{approvedCount}}</strong><small>已审核术语</small></article></div>
